...
if (isset($_GET["bene"])) {
    ...
    if ($b) {
        while ($d = mysqli_fetch_assoc($b)) {
            $e = array($d["nam"], $d["bank"], $d["acc"], $d["i"]);
            array_push($aa, $e);
        }
    }
...
if (isset($_GET["recent"])) {
    ...
    if ($b) {
        while ($d = mysqli_fetch_assoc($b)) {
            $e = array($d["id"], $d["name"], $d["amount"], $d["date"], $d["type"], $d["sender"]);
            array_push($aa, $e);
        }
    }
...
if (isset($_GET["recenta"])) {
    ...
    $a = "select * from tran where idd='".id()."' and type=1 order by id desc limit 5";
...
if (isset($_GET["recents"])) {
    ...
    $a = "select * from tran where idd='".id()."' and type=0 order by id desc limit 5";
...
if (isset($_GET["usr"])) {
    ...
    if ($b) {
        while ($d = mysqli_fetch_assoc($b)) {
            $e = array($d["idd"], $d["fnam"], $d["lnam"], $d["pass"], $d["email"], $d["amt"], $d["ver"]);
            array_push($aa, $e);
        }
    }
...
if (isset($_GET["gpt"])) {
    ...
    if ($b) {
        while ($d = mysqli_fetch_assoc($b)) {
            $e = array($d["i"], $d["nam"], $d["bank"], $d["acc"], $d["amt"], $d["date"]);
            array_push($aa, $e);
        }
    }
...
if (isset($_POST["edp"])) {
    ...
    $a = "update user set fnam='$fnam', lnam='$lnam', email='$eml', occ='$occ', phone='$pho', city='$city', state='$state', address='$add', dob='$dob' where idd='".id()."'";
...
if (isset($_POST["savc"])) {
    ...
    $a = "update user set country='$cou', type='$type', sex='$gender' where idd='".id()."'";
...
if (isset($_POST["otra"])) {
    ...
    $a = "insert into obank values('null','".id()."', '$acc', '$bank', '$accn', '$amt', '$date')";
if (gett("ver")=="1"){
    $b = mysqli_query($con, $a);
    if ($b) {
...
if (isset($_POST["upb"])) {
    ...
    $a = "update ben set nam='$acc', bank='$bank', acc='$accn' where id='".id()."'";
if (gett("ver")=="1"){
    $b = mysqli_query($con, $a);
    if ($b) {
...
if (isset($_POST["crb"])) {
    ...
    $a = "insert into ben values('null','".id()."', '$acc', '$bank', '$accn')";
if (gett("ver")=="1"){
    $b = mysqli_query($con, $a);
    if ($b) {
        header("location: beneficiary.php?sucx");
...
if (isset($_GET["card"])) {
    ...
    if ($b) {
        while ($d = mysqli_fetch_assoc($b)) {
            $e = array($d["id"], $d["name"], $d["num"], $d["date"], $d["cvv"], $d["type"], $d["idd"]);
            array_push($aa, $e);
        }
    }
    ...
    if (strlen($filt) >= 1) {
        ...
        if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" && $imageFileType != "webp") {
            ...
            if ($_FILES["pimg"]["size"] > 500000) {
                header("location: settings.php?erri=Image size too large(>500kb)");
            } else {

                if (!file_exists($fil)) {

                    if (move_uploaded_file($_FILES["pimg"]["tmp_name"], $fil)) {
                        $d = "update user set img='/".$fil."' where idd='".id()."'";
